...
> **Note on syntax:** Commands below are shown in CLI form (e.g. `snapshot -i`, `click <sel>`). Pass them to the browser tool as an argv array — one argument per element, unquoted: `["snapshot", "-i"]`, `["click", "@e3"]`, `["fill", "@e3", "Jane Doe"]`.
