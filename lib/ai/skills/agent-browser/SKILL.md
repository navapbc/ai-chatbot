...
1. **Navigate**: `{ action: "open", url: "<url>" }` (already waits for page load internally — do NOT add a separate waitforloadstate)
...
**NEVER use `wait` with `state` after clicks, fills, types, or other in-page interactions.** It wastes a tool call. Only use it after navigating to a completely new URL via a link or form submission, and only if the page has heavy async content that loads after the initial page load.
...
`find label` causes strict-mode violations whenever a label appears more than once. On benefit forms, "First Name", "State", "Yes", "No" etc. appear for applicant, representative, mailing address, and household — always causing failures.

**NEVER use `find label` for**: "Yes", "No", "Male", "Female", "First Name", "Last Name", "Street Address", "City", "State", "Zip Code", "Birthdate", "Phone". **NEVER include** asterisks (`*`) or colons (`:`) in the label text.
```
...
Empty/minimal snapshots mean a modal is blocking — NOT that snapshots are broken. Do NOT use `eval` to probe when snapshots are empty. Follow these steps:
...
3. **If ALL scoped snapshots are empty** — the modal lacks ARIA attributes (common on React apps like BenefitsCal). Use ONE evaluate to find it:
   ```
   { action: "eval", script: "document.querySelector('[aria-modal=true], .modal, [role=dialog]')?.outerHTML?.substring(0, 2000) || document.querySelector('body > div:not([aria-hidden])').outerHTML.substring(0, 2000)" }
   ```
...
**Google Translate bar** blocking clicks: `{ action: "eval", script: "document.querySelector('.VIpgJd-yAWNEb-hvhgNd')?.remove()" }`
...
```
{ action: "eval", script: "document.querySelector('.header').click();" }
```
Then wait for animation: `{ action: "wait", timeout: 700 }`

### When the submit button is disabled

Follow these steps IN ORDER. Do NOT use `eval` to probe or debug before completing steps 1–3.
...
- NEVER use `eval` to find, click, fill, select, or check elements — use proper actions
- NEVER use `eval` as a fallback when snapshots return empty/minimal content. Empty snapshots mean a modal is blocking — find and dismiss the modal.
